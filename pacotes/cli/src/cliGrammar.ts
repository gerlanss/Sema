...
const FONTES_IMPORTACAO = new Set([
  ...
  "angular-consumer", "flutter-consumer", "sveltekit-consumer", "nuxt-consumer", "firebase", "typescript",
  "python", "dart", "dotnet", "java", "go", "rust", "cpp", "php",
]);
...
function validarResumo(args: readonly string[], promptCurto: boolean): boolean {
  const parsed = parsearArgumentos(args, {
    opcoes: promptCurto ? ["--para"] : ["--para", "--saida", "--drift", "--cache", "--pedido"],
    flags: promptCurto ? ["--micro", "--curto", "--medio"] : ["--micro", "--curto", "--medio", "--raiz", "--com-drift", "--pacote", "--somente-pacote"],
    maxPosicionais: 1,
  });
  if (!parsed) return false;
...
function validarComandoBasico(comando: string, args: readonly string[]): boolean {
  ...
  switch (comando) {
    ...
    case "contexto-ia": {
      const parsed = parsearArgumentos(args, { opcoes: ["--saida", "--capacidade"], flags: ["--pacote"], minPosicionais: 1, maxPosicionais: 1 });
      const capacidade = parsed ? valorOpcao(parsed, "--capacidade") : undefined;
      return parsed !== null && (!capacidade || CAPACIDADES_IA.has(capacidade));
    }
    case "compilar":
    case "testar": {
    ...
    case "drift": {
      const parsed = parsearArgumentos(args, { opcoes: ["--escopo", "--cache", "--drift"], flags: ["--incluir-worktrees", "--incluir-consumidores-laterais", "--pacote"], maxPosicionais: 1 });

...
function gerarCabecalhoSemaDart(modulo: IrModulo): string {
  return [
    "// SEMA-GOVERNED",
    ...
    "// Descrição: artefato Dart gerado para humanos e IAs entenderem rapidamente a intenção do contrato.",
    "",
  ].join("\n");
}
...
export function gerarDart(modulo: IrModulo): ArquivoGerado[] {
  ...
  const tasks = modulo.tasks.map((task) => `// Task ${task.nome}: input=${task.input.length} output=${task.output.length} effects=${task.efeitosEstruturados.map((efeito) => `${efeito.categoria}:${efeito.alvo}`).join(", ") || "nenhum"} impl=${task.implementacoesExternas.map((impl) => `${impl.origem}:${impl.caminho}[${impl.statusImpl ?? "nao_verificado"}]`).join(", ") || "nenhuma"}`).join("\n");
  ...
  const codigo = `${gerarCabecalhoSemaDart(modulo)}${interops ? `${interops}\n` : ""}\n${tiposExternos}\n\n${enums}\n${entidades}\n${tasks}\n${flows}\n${routes}\n`;

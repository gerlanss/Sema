...
function gerarCabecalhoSemaJavaScript(modulo, tipo = "contrato") {
  const descricoes = {
    contrato: "artefato JavaScript gerado para executar e revisar as regras declaradas no contrato Sema.",
    teste: "testes JavaScript gerados a partir dos casos do contrato Sema.",
  };
  return [
    "// SEMA-GOVERNED",
...
export function gerarJavaScript(modulo: IrModulo): ArquivoGerado[] {
  ...
  const codigo = `${gerarCabecalhoSemaJavaScript(modulo)}${interoperabilidades ? `${interoperabilidades}\n` : ""}\n${tiposExternos}\n${tipos}\n${entidades}\n${enums}\n${states}\n${flows}\n${routes}\n${tasks}\n${contratosPublicos}\n`;
  const testes = `${gerarCabecalhoSemaJavaScript(modulo, "teste")}${gerarTestes(modulo)}`;

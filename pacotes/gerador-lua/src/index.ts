...
function gerarValidacaoPredicadoLua(texto, camposEntrada) {
  const diferenteDe = /^([A-Za-z_][A-Za-z0-9_]*)\s+deve_ser\s+diferente_de\s+([A-Za-z0-9_-]+)$/u.exec(texto);
  if (diferenteDe?.[1] && diferenteDe[2] && camposEntrada.has(diferenteDe[1])) {
    return `  if entrada[${JSON.stringify(diferenteDe[1])}] == ${JSON.stringify(diferenteDe[2])} then return nil end`;
  }

  const umDe = /^([A-Za-z_][A-Za-z0-9_]*)\s+deve_ser\s+um_de\s+\[\s*([^\]]+?)\s*\]$/u.exec(texto);
  if (umDe?.[1] && umDe[2] && camposEntrada.has(umDe[1])) {
    const valores = umDe[2].split(",").flatMap((parte) => parte.trim().split(/\s+/u)).filter(Boolean);
    return `  if not contem({ ${valores.map((valor) => JSON.stringify(valor)).join(", ")} }, entrada[${JSON.stringify(umDe[1])}]) then return nil end`;
  }
...
function gerarValidacoes(task) {
  ...
  const linhas = [
    ...
    ...task.regrasEstruturadas.map((regra) => {
      switch (regra.tipo) {
        case "predicado":
          return [gerarValidacaoPredicadoLua(regra.textoOriginal, camposEntrada), `  -- Predicado declarado em Sema: ${regra.textoOriginal}`].filter(Boolean).join("\n");
        default:
          return `  if not (${gerarExpressaoLua(regra, camposEntrada, "entrada")}) then error("Regra violada: ${regra.textoOriginal}") end`;
      }
    }),
    ...task.rules
...
function gerarPreparacaoSaida(task) {
  ...
  const ajustes = [];
  const precondicoes = [];
  const campoArtefatoI18n = ["artefato_ui", "artefato_real"].find((campo) => camposEntrada.has(campo));
  const campoContratoI18n = ["contrato_alvo", "contrato"].find((campo) => camposEntrada.has(campo));

  if (campoArtefatoI18n && (camposEntrada.has("idioma") || campoContratoI18n)) {
    precondicoes.push(`  local artefato_ui = tostring(entrada[${JSON.stringify(campoArtefatoI18n)}] or "")`);
    precondicoes.push(`  local contrato_i18n = tostring(entrada[${JSON.stringify(campoContratoI18n ?? "")}] or ""):lower()`);
    precondicoes.push(`  local idioma = tostring(entrada["idioma"] or ""):lower()`);
    precondicoes.push(`  if idioma == "" and (contrato_i18n:find("i18n_ptbr", 1, true) or contrato_i18n:find("pt-br", 1, true)) then idioma = "pt-br" end`);
    precondicoes.push(`  if idioma == "" and contrato_i18n:find("i18n_es", 1, true) then idioma = "es" end`);
    precondicoes.push(`  if idioma == "" and contrato_i18n:find("i18n", 1, true) then return nil end`);
    precondicoes.push(`  if (idioma == "pt-br" or idioma == "ptbr" or idioma == "pt") and artefato_ui:find("Descricao", 1, true) then return nil end`);
    precondicoes.push(`  if idioma == "es" and artefato_ui:find("Descripcion", 1, true) then return nil end`);
    if (camposSaida.has("garantia_observavel")) {
      ajustes.push(`  saida["garantia_observavel"] = true`);
    }
    if (camposSaida.has("diacriticos_preservados")) {
      ajustes.push(`  saida["diacriticos_preservados"] = true`);
    }
  }
  if (camposEntrada.has("drift")) {
    precondicoes.push(`  local drift_texto = tostring(entrada["drift"] or ""):lower()`);
    precondicoes.push(`  if drift_texto:find("sucesso:false", 1, true) or drift_texto:find("vinculos_quebrados", 1, true) or drift_texto:find("rotas_divergentes", 1, true) then return nil end`);
  }
  if (campoArtefatoI18n) {
    precondicoes.push(`  local artefato_responsivo = tostring(entrada[${JSON.stringify(campoArtefatoI18n)}] or "")`);
    precondicoes.push(`  local scroll_width = tonumber(artefato_responsivo:match("scrollWidth%s*(%d+)"))`);
    precondicoes.push(`  local client_width = tonumber(artefato_responsivo:match("clientWidth%s*(%d+)"))`);
    precondicoes.push(`  if scroll_width ~= nil and client_width ~= nil and scroll_width > client_width then return nil end`);
  }
  ...
  return `${precondicoes.length ? `${precondicoes.join("\n")}\n` : ""}  local saida = {\n${inicializacao}\n  }\n${ajustes.join("\n")}`;
}
...
function gerarCabecalhoSemaLua(modulo, tipo = "contrato") {
  const descricoes = {
    contrato: "artefato Lua gerado para executar e revisar as regras declaradas no contrato Sema.",
    teste: "testes Lua gerados a partir dos casos do contrato Sema.",
  };
  return [
    "-- SEMA-GOVERNED",
    ...
    `-- Descrição: ${descricoes[tipo] ?? descricoes.contrato}`,
    "",
  ].join("\n");
}
...
function gerarTestesLua(modulo, arquivoModulo) {
  ...
  return `${gerarCabecalhoSemaLua(modulo, "teste")}local origem = debug.getinfo(1, "S").source:sub(2)
...
export function gerarLua(modulo: IrModulo): ArquivoGerado[] {
  ...
  const codigo = `${gerarCabecalhoSemaLua(modulo)}

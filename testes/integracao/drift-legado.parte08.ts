...
const RAIZ_CACHE_SENTINELA = path.join(os.tmpdir(), `sema-drift-legado-cache-none-parte08-${process.pid}`);
...
test("cli drift resolve impls Lua em fixture sintetico", async () => {
  const base = await mkdtemp(path.join(os.tmpdir(), "sema-drift-lua-"));

  try {
    await criarProjetoLuaBridge(base);
    ...
    assert.equal(json.rotas_divergentes.length, 0);
    const caminhos = new Set(json.impls_validos.map((impl: { caminho: string }) => impl.caminho));
    assert.equal(caminhos.has("app.social.publicar"), true);
    assert.equal(caminhos.has("app.social.preparar_payload"), true);
  } finally {
    await rm(base, { recursive: true, force: true });
  }
});

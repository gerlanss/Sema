...
test("formatador preserva modificador required em campo chamado caminho", () => {
  const codigo = `
module exemplo.campo.caminho {
  task validar {
    input {
      caminho: Texto required
    }
    output {
      permitido: Booleano
    }
...
`;
  ...
  const resultado = formatarCodigo(codigo, "campo-caminho.sema");

  assert.equal(temErros(resultado.diagnosticos), false);
  assert.match(resultado.codigoFormatado ?? "", /input \{\n\s+caminho: Texto required\n\s+\}/);
});

test("formatador nao emite espaco final em flow multilinha", () => {
  const codigo = `
module exemplo.flow.sem_espaco_final {
  task executar {
    input {
      valor: Texto required
    }
    output {
      ok: Booleano
    }
  ...
  flow principal {
    valor: Texto
    etapa executar usa executar com
    valor = valor,
    ok = verdadeiro
    em_sucesso finalizar
  }
}
`;

  const resultado = formatarCodigo(codigo, "flow-multilinha.sema");

  assert.equal(temErros(resultado.diagnosticos), false);
  assert.doesNotMatch(resultado.codigoFormatado ?? "", /[ \t]$/m);
});
...
test("formatador preserva operadores compostos em regras e garantias", () => {
  const codigo = `
module exemplo.operadores.compostos {
  task medir {
    input {
...
`;
  ...
  const resultado = formatarCodigo(codigo, "operadores.sema");

  assert.equal(temErros(resultado.diagnosticos), false);
  assert.match(resultado.codigoFormatado ?? "", /total >= 0/);
  assert.match(resultado.codigoFormatado ?? "", /total <= 100/);
  assert.match(resultado.codigoFormatado ?? "", /total != 13/);
  assert.doesNotMatch(resultado.codigoFormatado ?? "", /> =/);
});
...
`;
  ...
  assert.match(resultado.codigoFormatado ?? "", /use lua app\.social\.handlers/);
});
